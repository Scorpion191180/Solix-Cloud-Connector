...
class DashboardSchematicTests(unittest.TestCase):
    def test_schematic_contains_every_energy_component(self) -> None:
        ...
        self.assertIn("style.css?v=20260911-148", dashboard)
        self.assertIn("house.js?v=20260911-148", dashboard)
        self.assertIn("app.js?v=20260911-148", dashboard)
    ...
    def test_virtual_house_is_touch_enabled_and_uses_live_data(self) -> None:
        ...
        self.assertIn('id="houseWeatherLocation"', dashboard)
        self.assertIn('id="houseGpsButton"', dashboard)
        self.assertIn('fetch(endpoint, request)', app_script)
        self.assertIn('"/api/weather/location"', app_script)
        self.assertIn('WEATHER_LOCATION_STORAGE_KEY = "solix-weather-location-v1"', app_script)
        self.assertIn("function loadStoredWeatherLocation", app_script)
        self.assertIn("saveActiveWeatherLocation(latestWeatherData.location_label)", app_script)
        ...
        self.assertIn('className: "bin rest"', app_script)
        self.assertIn('short: "♻"', app_script)
        ...
        self.assertIn("const targetHeight = 0.86", script)
        self.assertIn('? 1.38 : 1.30', script)
        self.assertIn("nextFoodVisitAt", script)
        self.assertIn("const setCareBone", script)
        self.assertNotIn("neck.rotation.x += 0.52", script)
        self.assertIn("function prepareDogClip", script)
        self.assertIn('!/(^|[.])Ear(?:_Tip)?_[LR][.]/i.test', script)
        self.assertIn("for (const word of loweredWords)", script)
        self.assertIn('new THREE.Vector3(\n            -6.15, 0,', script)
        self.assertIn('dogState.navigation === "dog-food" ?', script)
        self.assertIn("speed * alignment * delta", script)
        self.assertIn("dogStateUnchanged", script)
        self.assertIn("pose.dogStateApplied = true", script)
    ...
    def test_mobile_render_profile_and_house_builder_are_available(self) -> None:
        ...
        self.assertIn('const APP_BUILD_VERSION = "148"', script)
        self.assertIn("Version 148", dashboard)
        ...
        self.assertIn('function orthogonalWallPoint(start, point, useWallGrid = true)', script)
        self.assertIn('function snappedWallGridCorner(point)', script)
        ...
        self.assertIn('support.x, sample.z - support.z) <= 1.05', script)
        ...
        self.assertIn('function snappedFloorCellCenter(point)', script)
        self.assertIn('const width = Math.abs(end.x - start.x) + 1', script)
        ...
        self.assertIn('id: "floor-terrazzo"', script)
        self.assertIn('id: "wall-wallpaper-linen"', script)
        self.assertIn('id: "wall-wallpaper-botanical"', script)
        self.assertIn('function builderSurfaceTexture(surface, color)', script)
        self.assertIn('const roofWidth = quarterTurns % 2 ? worldRoofDepth : worldRoofWidth', script)
        self.assertIn('{ rotation: [0, 0, -side * pitch]', script)
        self.assertIn('const addHipFace = (points)', script)
        self.assertNotIn('new THREE.ConeGeometry(1, rise, 4)', script)
